#!/bin/bash
# Peckish smoke runner: build instrumented, run headless in the Playdate
...
SHOT="$ROOT/build/peckish-shot.png"
...
rm -rf "$DATA" "$SHOT"
("$SIM" "$ROOT/out/PeckishSmoke.pdx" >"$ROOT/build/sim.log" 2>&1 &)
